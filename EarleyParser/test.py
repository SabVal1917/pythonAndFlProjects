...
class Parser(ABC):
    def _parse_grammar(self, number) -> Grammar:
        with open(self.path / self.filename.format(number)) as file:
            ...
            return Grammar(rules_list, nt_number, alph_size, grammar_rules_number, old_start_terminal, end_terminal,
...
class TestAppropriateGrammars(unittest.TestCase, Parser):
    ...
    def test_six(self):
        ...
        for i in range(10):
            ...
            for j in range(10):
                another_list = list("a" * random.randint(0, 50) + "b" * random.randint(0, 50))
